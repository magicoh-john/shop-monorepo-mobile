...
- **콘텐츠**
  ...
  - 로그인 실패 알림(카드 상단, URL 쿼리 에러 기반 조건부 노출): "이미 이메일로 가입된 계정입니다. 이메일로 로그인해주세요." / "카카오 로그인 시 이메일 제공에 동의해주세요."
  - 자격증명 로그인 실패 알림(브라우저 alert, 이메일/비밀번호 불일치 시): "로그인 실패: 이메일 또는 비밀번호를 확인해주세요."
...
- 자격증명 로그인 실패 시 브라우저 네이티브 `alert()`로 메시지가 노출되는 것은 임시 구현이며 디자인 토큰 적용 대상이 아니다(브라우저 기본 UI). 추후 토스트/인라인 컴포넌트로 교체 시 명세를 갱신한다.
