...
1. `apps/web`이 켜져 있는지 확인한다 (꺼져 있다면 루트에서 `pnpm dev`).
2. 루트에서 Expo 개발 서버를 켠다.
   ```bash
   # 📍 위치: C:\project\shop-monorepo-mobile (루트)
...
3. 스마트폰 Expo Go 앱으로 QR코드를 다시 스캔한다 — STEP 1의 "Welcome!" 기본 화면 대신,
   방금 만든 로그인 화면(이메일/비밀번호 입력창 + "로그인" 버튼)이 보여야 한다.
4. 이메일 칸에 `admin@test.com`, 비밀번호 칸에 `123456`을 입력하고 "로그인" 버튼을 누른다
...
6. 이메일/비밀번호를 일부러 틀리게 입력해서 에러 문구("이메일 또는 비밀번호가 올바르지
