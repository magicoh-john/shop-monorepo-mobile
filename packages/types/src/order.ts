 ...
 * 주문 관련 공통 타입
 *
 * Order는 결제 완료 후 PostgreSQL DB에 저장되는 주문 데이터다.
 * 장바구니(CartItem)는 Redis에 저장되어(TTL 7일) 서버에도 보관되지만, 주문과 달리 영구 보존되지 않고
 * 취소/상태 변경 같은 주문 라이프사이클이 없다.
 *
 * 관련 타입:
 * - CartItem (cart.ts): 결제 전 Redis에 저장되는 임시 장바구니 상태
