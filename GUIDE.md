...
이 튜토리얼은 서버 두 개를 **동시에** 켜놓고 진행한다 — `apps/web`(Next.js 백엔드)과
`apps/mobile`(Expo 개발 서버)은 같은 모노레포 안에 있어도 별개의 프로세스라, 하나가 꺼지면
다른 하나도 정상 동작하지 않는다.

Windows Terminal에서 `Ctrl+Shift+T`(또는 탭 줄의 `+` 버튼)로 새 탭을 하나 더 연다 — 창을
새로 띄우는 대신 탭으로 열면 한 화면에서 두 서버 로그를 같이 볼 수 있다.

| 터미널 | 명령 (위치: 루트) | 언제 켜나 | 언제까지 켜두나 |
|---|---|---|---|
| 탭 1 | `pnpm dev` | 사전 준비 단계(지금)부터 | 실습이 끝날 때까지 계속 |
| 탭 2 | `pnpm --filter mobile start` | STEP 1에서 `apps/mobile`을 만든 뒤부터 | 실습이 끝날 때까지 계속 |

> 탭 2는 아직 `apps/mobile`이 없어서 지금은 실행할 수 없다 — STEP 1을 마치고 켠다.

두 서버 다 정상적으로 켜졌는지는 터미널 출력으로 확인한다.

- 탭 1(web): `▲ Next.js 16.2.6` 로그와 `Local: http://localhost:3000` 같은 줄이 보이면 정상이다.
- 탭 2(mobile): QR코드와 함께 `Metro waiting on ...` 줄이 보이면 정상이다.

둘 중 하나라도 에러를 내며 멈춰 있으면, 그 탭을 닫지 말고 에러 메시지를 그대로 확인한다 — 다시
실행하기 전에 어떤 에러인지 읽는 습관이 디버깅 시간을 줄여준다.
