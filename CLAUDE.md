...
> **시드 계정 사용 원칙**: 역할별(관리자/일반 사용자) 테스트 계정을 시드 스크립트로 미리 만들어 DB에 저장해둔다. 매번 회원가입부터 다시 거치지 않고 고정된 계정으로 즉시 로그인해 테스트할 수 있고, 자동화 테스트(E2E)도 같은 계정을 재사용해 안정적으로 반복 실행할 수 있다. 이건 교육용 단축법이 아니라 실무에서도 쓰이는 방식이다. 다만 **운영(production) 환경에는 이 시드 데이터를 절대 포함하지 않는다** — 비밀번호가 알려진 계정이 실 서비스에 존재하면 그 자체로 보안 취약점이 된다.
